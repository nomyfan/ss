...
---------------------------------------------------------------------------
Example: ./Measure-CodeLines.ps1 -Diretory C:\Users\Nomyfan\Desktop -Include *.ps1 -Exclude DHCP.ps1
#>
...
foreach ($dir in $Diretory) {
    $dir = $dir + "\*"
    $files = Get-ChildItem -Path $dir -Recurse -Filter $Filter -Include $Include -Exclude $Exclude
