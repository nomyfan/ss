...
Get-ChildItem $In -File | ForEach-Object -Parallel {
  $fullName = $_.FullName
  $name = $_.Name
  Start-Process -PassThru -Wait -FilePath 'docker' -ArgumentList 'run', '--rm', '-v', """${fullName}"":""/in/${name}""", '-v', """${using:Out}"":/out", 'nerdfonts/patcher:4.4.0', '--complete', '--careful', '--makegroups', '1'
